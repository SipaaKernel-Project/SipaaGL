...
namespace SipaaGL
{
	...
	public unsafe class Graphics : IDisposable
	{
		...
		/// <param name="Font">BitFont font to use.</param>
		/// <param name="Color">Color to draw with.</param>
		/// <param name="Center">Option to cented the text at X and Y.</param>
		public void DrawStringBF(int X, int Y, string Text, BitFont Font, Color Color, bool Center = false)
		{
		...
		public uint DrawChar(int X, int Y, char Char, BitFont Font, Color Color, bool Center)
		{
			uint Index = (uint)BitFont.DefaultCharset.IndexOf(Char);
			if (Char == ' ')
			{
				return Font.Size2;
			}
