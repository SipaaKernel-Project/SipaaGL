...
namespace SipaaGL {
    ...
    public unsafe static class TTFManager {
        ...
        private static Graphics prevCanv;
        ...
        public static GlyphResult RenderGlyphAsBitmap(string font, char glyph, Color color, float scalePx = 16) {
            ...
            var bmp = new Image((uint)image.Width, (uint)image.Height);

            for (int j = 0; j < image.Height; j++) {
                for (int i = 0; i < image.Width; i++) {
                    byte alpha = image.Pixels[i + j * image.Width];
                    bmp.Internal[i + j * image.Width] = ((uint)alpha << 24) + (uint)rgbOffset;
                }
            }
        ...
        public static void DrawString(this Graphics cv, Color col, string text, string font, float px, int x, int y, float spacingMultiplier = 1f) {
            prevCanv = cv;
            float offX = 0;
            float offY = 0;
            ...
            foreach (char c in text) {
                ...
                GlyphResult g = RenderGlyphAsBitmap(font, c, col, px);
                cv.DrawImage(x + (int)offX, y + g.offY, g.bmp, true);
                offX += g.offX;
            }
        }
    ...
    public struct GlyphResult {
        ...
        public GlyphResult(Image bmp, int offX, int offY) {
            this.bmp = bmp;
            this.offX = offX;
            this.offY = offY;
        }
