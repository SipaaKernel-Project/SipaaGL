...
namespace SipaaGL {
    ...
    public unsafe static class TTFManager {
        ...
        private static VBECanvas prevCanv;
        ...
        public static void DrawString(this VBECanvas cv, Color col, string text, string font, float px, int x, int y, float spacingMultiplier = 1f) {
            prevCanv = cv;
            float offX = 0;
            float offY = 0;
            ...
            foreach (char c in text) {
                ...
                cv.DrawImage(x + (int)offX, y + g.offY, g.bmp);
                offX += g.offX;
            }
        }
